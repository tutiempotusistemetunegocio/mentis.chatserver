...
## Cambio (6/9/2026): reels sin voz, y opción de dos clips ("dosPartes") cuando el ángulo lo pida

Pedido explícito de Rodrigo, después de descartar varias alternativas más complejas (voz con ElevenLabs, ensamblado con Creatomate, plataformas todo-en-uno como Fliki): "no quiero muchos videos con voz, prefiero videos con música, ambiente que generen interés, y con captions" — y "puede ser un simple reel de doce segundos utilizando mi imagen (...) o puedes hacer dos clips de doce segundos, parte uno, parte dos".

Con esto: el guion completo (`guion`) queda como registro/respaldo, no como texto para narrarse — el reel se publica mudo, con música y captions (esto último ya lo arma `daily-media.js` desde el 3/9/2026, ver `daily-media.md`). Lo nuevo acá es que Mentis ahora decide, día a día, `dosPartes`: si el ángulo de hoy entra en un solo clip de 12s, o si realmente tiene dos momentos que se benefician de separarse (el problema y la vuelta, el mito y la realidad, el antes y el después). Por defecto sigue siendo un solo clip — dos partes es la excepción, cada parte de más es un pedido real a Higgsfield (costo y tiempo), así que solo se justifica cuando de verdad suma claridad al gancho.

Cuando `dosPartes` es `true`, en vez de `escenaVisual`/`captionText` (un solo campo cada uno), Mentis devuelve `escenaVisualParte1`/`escenaVisualParte2` y `captionTextParte1`/`captionTextParte2` — Parte 1 planta el gancho/problema, Parte 2 es la resolución, con un texto en pantalla que funde el cierre con la invitación a comentar "MENTIS" (el CTA también aparece en el video, no solo en la descripción escrita del post). `musicStyle` sigue siendo un solo campo, se usa en las dos partes. El campo `cta` (para pegar como descripción real del post al publicar) no cambia y sigue siendo obligatorio siempre.
