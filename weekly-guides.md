...
## Bug real encontrado con guías reales (6/9/2026): texto invisible en los quiebres de página del PDF

Rodrigo lo reportó primero sobre la guía cero (`guia-cero.js`, ver `guia-cero.md`), pero el bug vive en `guide-pdf.js` — el mismo archivo que arma el PDF de estas guías del catálogo — así que aplica exactamente igual acá. Reportó una frase cortada justo en el borde de una página, con la continuación "sin aparecer en ningún lado" del PDF, y aclaró que no era un caso aislado: pasaba por toda la guía, en cualquier quiebre de página en medio de un párrafo largo.

Causa real, confirmada contra el código fuente de pdfkit (no se pudo instalar pdfkit en este entorno para probarlo corriendo — el registro de npm sigue bloqueado acá, mismo problema de siempre — así que se leyó el código de la librería en vez de adivinar): `drawPageBackground()` pinta el fondo de cada página nueva con `doc.rect().fill(COLOR_BG)`, y `fill(color)` llama por dentro a `fillColor(color)`, que guarda el color en `doc._fillColor` — una propiedad de JS simple, separada del verdadero estado gráfico de PDF que cubren `doc.save()`/`doc.restore()` (esos dos solo apilan la matriz de transformación, nunca tocan `_fillColor`). Cuando un párrafo largo no entra en una página, pdfkit agrega la página siguiente sola y, en su propia lógica interna, restaura el color del texto que sigue leyendo `doc._fillColor` — pero como `drawPageBackground()` ya lo pisó con el color de fondo antes de que corra esa restauración, el texto que sigue queda escrito en el mismo color que el fondo: no desaparece, queda invisible.

Corrección: `drawPageBackground()` ahora guarda `doc._fillColor` antes de pintar el fondo y lo devuelve después, a mano (`save()`/`restore()` no lo cubren). Mismo caveat de siempre con el código nuevo: no se pudo confirmar contra una corrida real todavía — la próxima guía (de este catálogo o la guía cero) que tenga un párrafo largo cruzando una página es la que lo confirma de verdad.

**Mismo caveat que el bug de las páginas en blanco**: esto no arregla los PDFs ya generados. Las guías (de este catálogo y la guía cero) generadas antes de este arreglo van a seguir con el texto invisible en sus quiebres de página hasta que se regeneren — para el catálogo, con la herramienta de la sección siguiente; para la guía cero, corriendo el workflow de nuevo.
...
**Límite honesto encontrado en auditoría (3/9/2026), no corregido a propósito**: `parseGuideMarkdown` reconstruye una cita textual (bloque `cita`) tomando solo la PRIMERA línea de texto entre comillas. Si alguna vez una cita real ocupa más de una línea (un salto de línea literal dentro del texto de la cita, algo que no se vio todavía en ninguna guía real), esa reconstrucción perdería el resto de la cita al regenerar el PDF. No afecta ninguna guía ya generada (el `.md`/PDF originales siempre salen completos, esto solo importa si algún día se regenera el PDF de una guía vieja que tenga justo este caso) — se deja documentado en vez de tocar el parser sin un caso real que lo justifique.

## Bug real encontrado en auditoría (3/9/2026): dos guías del mismo tipo podían pisarse entre sí

El id de cada guía (y el nombre de su archivo) se arma con fecha + tipo + título "slugificado". Nada impedía que dos guías del MISMO tipo generadas en la MISMA corrida (por ejemplo las 2 premium de un día) terminaran con el mismo id, si sus títulos daban el mismo slug una vez normalizados. Cuando eso pasaba, la segunda guía sobreescribía en disco el archivo de la primera antes de subir nada, y el catálogo quedaba con dos entradas de id idéntico apuntando al mismo archivo — la primera guía (ya generada y ya pagada como llamada a la API) desaparecía en silencio, sin ningún error visible. Corrección: si el id ya existe en el catálogo (de esta corrida o de una anterior), se le agrega un sufijo corto al azar antes de guardar nada.
