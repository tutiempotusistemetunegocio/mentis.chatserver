...
- **Guía cero** (agregada 6/9/2026, pedido explícito de Rodrigo: "esa guía cero de referencia para enviar siempre al cliente, siempre que responde un CTA"): arriba de todo, separada del catálogo — la guía de referencia FIJA que explica el sistema completo, armada por `guia-cero.js` (ver `guia-cero.md`). A diferencia de las guías de abajo, siempre es la MISMA (se sobreescribe cada vez que se regenera, nunca acumula versiones) y se dispara a mano desde GitHub Actions, no sola cada semana. Muestra el título/subtítulo actual, cuándo se generó por última vez, y los links al texto completo y al PDF.
...
- **Contenido reciente**: los últimos guiones (reel/carrusel/podcast), las últimas fotos elegidas, y el prompt exacto que se le mandó a Higgsfield para cada video (pedido explícito de Rodrigo, "no veo el prompt del video" — antes se armaba y se mandaba sin dejar rastro visible en ningún lado). Un día normal muestra una tarjeta de prompt; un día de "dos partes" (6/9/2026, ver `daily-script.md`) muestra dos, una por cada clip.
